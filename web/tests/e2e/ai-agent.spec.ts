...
test.describe('AI Canvas Agent', () => {
  test.describe('Basic AI Tools', () => {
    test('create a red rectangle with AI', async ({ authenticatedPage, roomId }) => {
      ...
      await expect(authenticatedPage.locator('text="AI is thinking"')).toBeVisible();

      // Wait for completion (timeout 10s)
      await authenticatedPage.waitForTimeout(10000);

      // Check history for success
      const historySection = authenticatedPage.locator('text=/Created|shape|rectangle/i');
      await expect(historySection.first()).toBeVisible({ timeout: 5000 });
    });

    test('create a blue circle with AI', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
    ...
    test('AI command shows loading state', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      await expect(authenticatedPage.locator('text="AI is thinking"')).toBeVisible({ timeout: 1000 });
    });
  });
  ...
  test.describe('Advanced AI Tools', () => {
    ...
    test('change shape color with AI', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
  ...
  test.describe('Complex AI Commands', () => {
    test('create a login form (multi-shape creation)', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
    ...
    test('create a navigation bar with 4 items', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
    ...
    test('create a 3x3 grid of circles', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
    ...
    test('complex command completes within 15 seconds', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      const aiTextarea = authenticatedPage.getByPlaceholder(/ask ai/i);
      await aiTextarea.fill('Create a dashboard with 5 widgets arranged in a grid');
  ...
  test.describe('AI History & Collaboration', () => {
    test('AI history shows all commands', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      const historySection = authenticatedPage.locator('.historyList, [class*="history"]');
      const entries = historySection.locator('[class*="historyEntry"], [class*="entry"]');
      await expect(entries).toHaveCount(2, { timeout: 5000 });
    });

    test('history displays user name, timestamp, prompt, response', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      await expect(authenticatedPage.locator('text="You"')).toBeVisible({ timeout: 5000 });

      // Check for timestamp (relative time like "just now" or "2s ago")
      await expect(authenticatedPage.locator('text=/ago|just now/i')).toBeVisible({ timeout: 5000 });

      // Check for prompt
      await expect(authenticatedPage.locator('text="Create a rectangle"')).toBeVisible();
    });

    test('history persists across page refresh', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      await expect(authenticatedPage.locator('text="Create a test rectangle"')).toBeVisible({ timeout: 5000 });
    });

    test('guest users see history but cannot send commands', async ({ guestPage, authenticatedPage, roomId }) => {
      ...
      await expect(guestPage.locator('text="Create a rectangle for testing"')).toBeVisible({ timeout: 5000 });
    ...
    test('multi-user AI history sync - user A sends command, user B sees it', async ({ multiUserContext, roomId }) => {
      ...
      await expect(user2.locator('text="Create a shared rectangle"')).toBeVisible({ timeout: 3000 });
    });
  });

  test.describe('AI Error Handling', () => {
    test('invalid command shows error', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
    ...
    test('prompt too long (>1000 chars) is rejected', async ({ authenticatedPage, roomId }) => {
      await authenticatedPage.goto(`/c/main?roomId=${roomId}`);
      await authenticatedPage.waitForLoadState('networkidle');
      ...
      await expect(authenticatedPage.locator('text=/too long|maximum/i')).toBeVisible({ timeout: 2000 });
    });

    test('guest user sees disabled input with tooltip', async ({ guestPage }) => {
