...
test.describe('Authentication & Authorization', () => {
  ...
  test('guest user cannot create shapes (toolbar buttons disabled)', async ({ page }) => {
    await page.goto('/c/main');
    await page.waitForLoadState('networkidle');
  ...
  test('authenticated user can sign in', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /rectangle/i })).toBeEnabled();
    await expect(page.getByRole('button', { name: /circle/i })).toBeEnabled();
    await expect(page.getByRole('button', { name: /text/i })).toBeEnabled();
  });

  test('authenticated user can create shapes', async ({ authenticatedPage }) => {
    // Rectangle button should be enabled
    await expect(authenticatedPage.getByRole('button', { name: /rectangle/i })).toBeEnabled();
  ...
  test('authenticated user can use AI assistant', async ({ authenticatedPage }) => {
    ...
    const submitButton = authenticatedPage.getByRole('button', { name: /send/i });
  ...
  test('editor vs guest role enforcement', async ({ authenticatedPage, guestPage, roomId }) => {
    ...
    await expect(guestPage.getByRole('button', { name: /rectangle/i })).toBeDisabled();
