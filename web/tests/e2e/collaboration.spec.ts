...
test.describe('Real-Time Collaboration', () => {
  test.describe('Cursor Presence', () => {
    test('user A moves mouse, user B sees cursor in real-time', async ({ multiUserContext, roomId }) => {
    ...
    test('multiple users cursors visible simultaneously', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
  ...
  test.describe('Shape Sync', () => {
    test('user A creates shape, user B sees it instantly', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
    ...
    test('user A moves shape, user B sees movement in real-time', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
    ...
    test('user A deletes shape, user B sees deletion', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
  ...
  test.describe('Multi-User Editing', () => {
    test('two users create shapes simultaneously', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
    ...
    test('two users move different shapes at same time', async ({ multiUserContext, roomId }) => {
      const { user1, user2 } = multiUserContext;
  ...
  test.describe('Persistence', () => {
    test('shapes persist after page refresh', async ({ authenticatedPage, roomId }) => {
