...
test.describe('Shape Creation & Editing', () => {
  test.describe('Rectangle Operations', () => {
    test('create rectangle with click-and-drag', async ({ authenticatedPage }) => {
    ...
    test('small rectangles (<10px) are not created', async ({ authenticatedPage }) => {
      await authenticatedPage.getByRole('button', { name: /rectangle/i }).click();
  ...
  test.describe('Circle Operations', () => {
    test('create circle with click-and-drag from center', async ({ authenticatedPage }) => {
    ...
    test('small circles (<5px radius) are not created', async ({ authenticatedPage }) => {
      await authenticatedPage.getByRole('button', { name: /circle/i }).click();
  ...
  test.describe('Text Operations', () => {
    test('create text shape by clicking canvas', async ({ authenticatedPage }) => {
      ...
      // Type text
      await authenticatedPage.fill('input[placeholder*="Enter text"]', 'Hello World');
    ...
    test('cancel text creation with Escape', async ({ authenticatedPage }) => {
      ...
      // Type some text
      await authenticatedPage.fill('input[placeholder*="Enter text"]', 'Cancel this');
    ...
    test('double-click text to edit', async ({ authenticatedPage }) => {
      ...
      await canvas.click({ position: { x: 400, y: 300 } });
      await authenticatedPage.waitForSelector('input[placeholder*="Enter text"]', { timeout: 2000 });
      await authenticatedPage.fill('input[placeholder*="Enter text"]', 'Original Text');
      await authenticatedPage.keyboard.press('Enter');
      await authenticatedPage.waitForTimeout(500);
  ...
  test.describe('Shape Properties', () => {
    ...
    test('click empty canvas deselects shape', async ({ authenticatedPage }) => {
      // Create and select a rectangle
      await authenticatedPage.getByRole('button', { name: /rectangle/i }).click();
      const canvas = authenticatedPage.locator('canvas').first();
    ...
    test('shapes persist across page refresh', async ({ authenticatedPage, roomId }) => {
